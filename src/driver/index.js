...
function deliver(orderId) {
  console.log("Driver finished delivery of orderId: ", events.id, orderId); //events is the websocket
  events.emit(EVENT_NAMES.delivered, orderId);
}

function handlePickup(event) {
  console.log("Driver received a pickup event!", events.id, event.orderId);  //in lab 12 this was event (without the .orderId)
  setTimeout(
